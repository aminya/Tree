...
Using the `DotGenerator.hpp` header, you can now also generate .DOT files for use in Graphviz. This means that you can now quickly and easily visualize the structure of the tree. This will obviously require Graphviz to be installed.

In order to generate a .DOT file, simply pass the Tree object to `DotGenerator::OutputToFile(...)`, along with the desired output path and filename. For example:
...
DotGenerator::OutputToFile(tree, "C:\\PathToFile\\TreeGraph.dot");
```
...
Once the .DOT file has been created, the visualization can be generated by running the following command from the command prompt:

```
$>dot -Tpng C:\PathToFile\TreeGraph.dot -O C:\PathToFile\TreeViz.png
```